...
def readDataFrame(path, file, df, lastDay, windowDiv, overlapTime):
            ...
            if(int(date[:2]) > 20): #is verified that year is over 2020
                ...
                if isAfter(currentDay,lastDay):
                    ...
                    for i in range(totalWindows):  ## it is iterated windowDiv times
                        ...
                        rawDataDiv = pd.DataFrame(df.loc[(x + i*(overlapLength)):(x + (i+2)*(overlapLength))]) ## it is iterated windowDiv times

                        features = getFeatures(rawDataDiv)    #getFeatures(rawDataDiv)

                        if(i > 0):
                            sec += round((overlapLength)*period*100) ##adjusts the current time
    ...
    for file in files:
        ...
        if file.endswith(".csv"):
            ...
            if content.empty :
                ...
                content = readDataFrame(path2, file, df, lastDay, 4, 1.25)

            else:

                content = content.append(readDataFrame(path2, file, df, lastDay, 4, 1.25),  ignore_index=True)
        ...
        if filesRead >= 1:
            ...
            if existFile == 1:
                content.to_csv(features_path + str(sensor) + ".csv", mode='a', index=False, header=False)
                content = pd.DataFrame()
            else:
                if not content.empty:
                    content.to_csv(features_path + str(sensor) + ".csv", index=False)
                    existFile = 1
                    content = pd.DataFrame()
...
features_path = directorio + 'RawFeatures1/' #D:/DatosOvejas/

n_jobs = multiprocessing.cpu_count() - 1

print(n_jobs)

#readSensor("2", raw_path, features_path)           ##   Debug Line
