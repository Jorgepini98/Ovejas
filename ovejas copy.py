        ...
        if newMov[0:5] == 'HORA_': #gets next new movement
            ...
            if(int(date[:2]) > 20):
                ...
                if isAfter(currentDay,lastDay):
                    ...
                    features = getData(data)

                    data = pd.DataFrame({'Sensor': [Sensor], 'Date': [date], 'Hour': [hour], 'Minutes': [min], "Seconds": [sec]})

                    dataX = data

                    # y = len(features["Acel_x"])

                    # print(y)

                    # print(data)

                    for x in range(len(features["l_Accel_x"])):
...
def readSensor(sensor, raw_path, features_path):
    ...
    for file in files:
        ...
        if file.endswith(".csv"):
            ...
            df = pd.read_csv(path2 + file, sep=';', names = ['HORA','Timestamp','l_Accel_x','l_Accel_y','l_Accel_z','l_Gyro_x','l_Gyro_y','l_Gyro_z','l_Magnet_x','l_Magnet_y','l_Magnet_z','h_Accel_x','h_Accel_y','h_Accel_z','h_Gyro_x','h_Gyro_y','h_Gyro_z','h_Magnet_x','h_Magnet_y','h_Magnet_z'],dtype = "string")
...
#readSensor('1', raw_path, features_path)
